"""
Silhouette USB Connection

Performs the required interactions with the Silhouette backend through pyusb and libusb.
"""
...
USB_LOCK_VENDOR = 0x0b4d # Graphtec vendor
USB_LOCK_PRODUCT = 0x1121 # Silhouette Cameo 1, for now
...
class USBConnection:
    ...
    def find_device(self, index=0):
        ...
        if len(devices) == 0:
            ...
            raise ConnectionRefusedError
        for d in devices:
            self.channel(_("Silhouette device detected:"))
            string = str(d)
        ...
        try:
            ...
            if self.backend_error_code == LIBUSB_ERROR_ACCESS:
                ...
                self.channel(_("Silhouette devices were found. But something else was connected to them."))
            else:
                self.channel(_("Silhouette devices were found but they were rejected by device criteria in Controller."))
            raise ConnectionRefusedError
        return device
